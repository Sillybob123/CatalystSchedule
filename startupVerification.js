...
(function() {
    ...
    console.log('%cSTARTUP VERIFICATION', 'color: #3b82f6; font-size: 16px; font-weight: bold;');
    ...
    if (typeof firebase !== 'undefined') {
        checks.push('OK Firebase SDK loaded');
    } else {
    ...
    if (typeof debugLog === 'function') {
        checks.push('OK Debug console loaded');
    } else {
        checks.push('Warning: Debug console NOT loaded (non-critical)');
    }
    
    // Check 3: Timestamp helper loaded
    if (typeof getTimestampValue === 'function') {
        checks.push('OK Timestamp helper loaded');
    } else {
    ...
    if (typeof getProjectState === 'function') {
        checks.push('OK State manager loaded');
    } else {
    ...
    if (typeof bulletproofNormalize === 'function') {
        checks.push('OK Bulletproof normalize loaded');
    } else {
    ...
    if (typeof setupBulletproofSubscriptions === 'function') {
        checks.push('OK Fixed subscriptions loaded');
    } else {
    ...
    // Display results
    console.log('%cVERIFICATION RESULTS:', 'color: #f59e0b; font-size: 14px; font-weight: bold;');
    checks.forEach(check => console.log(check));
    
    if (allPassed) {
        console.log('%c\nALL CRITICAL CHECKS PASSED\n', 'color: #10b981; font-size: 14px; font-weight: bold; background: #022c22; padding: 8px;');
        console.log('%cReady to test: Create a task or project and watch the debug console', 'color: #3b82f6; font-style: italic;');
        ...
        alert('Some scripts failed to load!\n\nPlease:\n1. Hard refresh (Cmd+Shift+R or Ctrl+Shift+R)\n2. Clear browser cache\n3. Check browser console (F12) for errors');
    }
    
    // Additional environment info
    console.log('\n%cENVIRONMENT INFO:', 'color: #8b5cf6; font-size: 14px; font-weight: bold;');
