...
// ==========================================
// ENHANCED STATUS REPORT GENERATOR
// Professional, Detailed Team Analytics
// ==========================================
function generateStatusReport() {
    ...
    const now = new Date();
    now.setHours(0, 0, 0, 0);
    // Initialize comprehensive user workload tracking
    const userWorkload = {};
    ...
    allProjects.forEach(project => {
        ...
        const completedTasks = Object.values(timeline).filter(t => t === true).length;
        const progressPercentage = totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0;
        // Determine next action needed
        let nextAction = 'Awaiting approval';
        let actionOwner = 'Admin';
        if (project.proposalStatus === 'approved') {
            if (!timeline["Interview Scheduled"] && project.type === 'Interview') {
                nextAction = 'Schedule interview';
                actionOwner = project.authorName;
            } else if (!timeline["Interview Complete"] && project.type === 'Interview') {
                nextAction = 'Complete interview';
                actionOwner = project.authorName;
            } else if (!timeline["Article Writing Complete"]) {
                nextAction = 'Write article';
                actionOwner = project.authorName;
            } else if (!project.editorId) {
                nextAction = 'Assign editor';
                actionOwner = 'Admin';
            } else if (!timeline["Review In Progress"]) {
                nextAction = 'Begin editor review';
                actionOwner = project.editorName || 'Editor';
            } else if (!timeline["Review Complete"]) {
                nextAction = 'Complete editor review';
                actionOwner = project.editorName || 'Editor';
            } else if (!timeline["Suggestions Reviewed"]) {
                nextAction = 'Review editor suggestions';
                actionOwner = project.authorName;
            } else {
        ...
        const projectInfo = {
            ...
            hasDeadlineRequest: (project.deadlineRequest && project.deadlineRequest.status === 'pending') ||
                               (project.deadlineChangeRequest && project.deadlineChangeRequest.status === 'pending'),
            timeline: timeline
        };
        ...
        if (project.authorId && userWorkload[project.authorId]) {
            ...
            else if (status === 'due-soon') userWorkload[project.authorId].dueSoon++;
            else userWorkload[project.authorId].onTrack++;
            userWorkload[project.authorId].totalWorkload++;
        }
        // Add to editor's workload if assigned
        if (project.editorId && userWorkload[project.editorId]) {
            ...
            else if (status === 'due-soon') userWorkload[project.editorId].dueSoon++;
            else userWorkload[project.editorId].onTrack++;
            userWorkload[project.editorId].totalWorkload++;
        }
    });
    // === PROCESS TASKS ===
    allTasks.forEach(task => {
        ...
        const taskInfo = {
            ...
            hasExtensionRequest: task.extensionRequest && task.extensionRequest.status === 'pending'
        };
        // Add to assignees' workload
        if (task.assigneeIds && Array.isArray(task.assigneeIds)) {
            task.assigneeIds.forEach(assigneeId => {
                if (userWorkload[assigneeId]) {
                    ...
                    else if (status === 'due-soon') userWorkload[assigneeId].dueSoon++;
                    else userWorkload[assigneeId].onTrack++;
                    userWorkload[assigneeId].totalWorkload++;
                }
            });
        } else if (task.assigneeId && userWorkload[task.assigneeId]) {
            userWorkload[task.assigneeId].tasks.push(taskInfo);
            if (status === 'overdue') userWorkload[task.assigneeId].overdue++;
            else if (status === 'completed') userWorkload[task.assigneeId].completed++;
            else if (status === 'due-soon') userWorkload[task.assigneeId].dueSoon++;
            else userWorkload[task.assigneeId].onTrack++;
            userWorkload[task.assigneeId].totalWorkload++;
        }
    });
    ...
    Object.values(userWorkload).forEach(user => {
        totalOverdue += user.overdue;
        totalOnTrack += user.onTrack;
        totalCompleted += user.completed;
        totalDueSoon += user.dueSoon;
    });
    allProjects.forEach(project => {
        if (project.proposalStatus === 'pending') totalPendingApproval++;
    });
    ...
    if (totalOverdue > 0) {
        recommendations.push({
            type: 'urgent',
            icon: '🚨',
            title: 'CRITICAL: Overdue Items Require Immediate Attention',
            message: `There are ${totalOverdue} overdue item(s) that need immediate action. Review and prioritize these items to get back on track.`
        });
    }
    // Items due soon
    if (totalDueSoon > 0) {
        recommendations.push({
            ...
            message: `${totalDueSoon} item(s) are due within the next 3 days. Ensure team members are aware and prepared to meet these deadlines.`
        });
    }
    // Pending approvals
    if (totalPendingApproval > 0) {
        recommendations.push({
            ...
            message: `${totalPendingApproval} project(s) are awaiting admin approval. Review proposals to keep the workflow moving.`
        });
    }
    // Team member workload warnings
    Object.values(userWorkload).forEach(user => {
        if (user.totalWorkload > 0) {
            if (user.overdue > 0) {
                recommendations.push({
                    type: 'urgent',
                    icon: '👤',
                    title: `${user.name} Has Overdue Items`,
                    message: `${user.name} has ${user.overdue} overdue item(s). Schedule a check-in to identify blockers and provide support.`
                });
            }
            if (user.totalWorkload > 8) {
                recommendations.push({
                    type: 'warning',
                    icon: '📊',
                    title: `${user.name} - High Workload`,
                    message: `${user.name} is assigned to ${user.totalWorkload} items. Consider redistributing work or extending deadlines if needed.`
                });
            }
        }
    });
    // Positive recommendations
    if (totalCompleted > 0 && totalOverdue === 0) {
        recommendations.push({
            type: 'success',
            icon: '🎉',
            title: 'Great Progress!',
            message: `The team has completed ${totalCompleted} item(s) with no overdue items. Keep up the excellent work!`
        });
    }
    // === BUILD THE ENHANCED REPORT HTML ===
    ...
    let html = `
        <div class="report-header">
            <h2>📊 Catalyst Team Status Report</h2>
    ...
    html += `
        <div class="report-section executive-summary">
            ...
            <div class="summary-grid">
                ...
                <div class="summary-item">
                    <div class="summary-icon">⏰</div>
                    <div class="summary-value">${totalDueSoon}</div>
                    <div class="summary-label">Due Soon (≤3 days)</div>
                </div>
    ...
    html += `
        <div class="report-section team-details">
            <h2>👥 Detailed Team Member Breakdown</h2>
    `;
    // Sort users: those with work first, then by overdue count
    const sortedUsers = Object.values(userWorkload)
        .filter(user => user.totalWorkload > 0)
        .sort((a, b) => {
    ...
    if (sortedUsers.length === 0) {
        html += `<p style="text-align: center; color: #64748b; padding: var(--space-8);">No active assignments found.</p>`;
    }
    sortedUsers.forEach(user => {
        // Determine card status class
        let cardStatusClass = '';
        if (user.overdue > 0) cardStatusClass = 'needs-attention';
        else if (user.totalWorkload > 0 && user.completed === user.totalWorkload) cardStatusClass = 'excellent';
        else if (user.onTrack > 0 || user.dueSoon > 0) cardStatusClass = 'in-progress';
        ...
        } else if (user.totalWorkload > 5) {
            workloadLevel = 'medium';
            workloadColor = '#f59e0b';
        }
        html += `
            <div class="user-card ${cardStatusClass}">
                <div class="user-card-header">
                    ...
                    <div class="workload-indicator ${workloadLevel}">
                        <div class="workload-count">${user.totalWorkload}</div>
                ...
                <div class="user-stats">
                    ...
                    <div class="stat-item on-track">
                        ...
                        <span class="stat-value">${user.onTrack + user.dueSoon}</span>
        ...
        if (user.projects.length > 0) {
            ...
            const sortedProjects = [...user.projects].sort((a, b) => {
                const urgencyOrder = { 'critical': 0, 'urgent': 1, 'high': 2, 'moderate': 3, 'normal': 4, 'completed': 5 };
            ...
            sortedProjects.forEach(project => {
                const deadlineText = project.deadline ?
                    `${new Date(project.deadline + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}` :
                    'No deadline';
                const daysText = project.daysUntilDeadline !== null ?
                    (project.daysUntilDeadline < 0 ?
                        `${Math.abs(project.daysUntilDeadline)} day(s) overdue` :
                        project.daysUntilDeadline === 0 ?
                        'Due today' :
                        `${project.daysUntilDeadline} day(s) remaining`) :
                    '';
                html += `
                    <div class="work-item ${project.status}" onclick="openDetailsModal('${project.id}')">
                        <div class="work-item-header">
                            <div class="work-item-title">
                                ${escapeHtml(project.title)}
                                ${project.hasDeadlineRequest ? ' ⏰' : ''}
                            </div>
                            <div class="work-item-type">${escapeHtml(project.type)}</div>
                        </div>
                        <div class="work-item-meta">
                            <div class="work-item-status">
                                <strong>Status:</strong> ${escapeHtml(project.state)}
                                 <span style="color: #94a3b8;">|</span>
                                <strong>Progress:</strong> ${project.progressPercentage}% (${project.completedTasks}/${project.totalTasks} tasks)
                            </div>
                        </div>
                        <div class="work-item-meta" style="margin-top: 8px;">
                            <div style="font-weight: 600; color: #475569;">
                                <strong>Next Action:</strong> ${escapeHtml(project.nextAction)}
                                ${project.actionOwner !== 'N/A' ? `<span style="color: #3b82f6;">(${escapeHtml(project.actionOwner)})</span>` : ''}
                            </div>
                        </div>
                        <div class="work-item-meta" style="margin-top: 8px;">
                            <div style="font-weight: 600; color: #64748b;">
                                Proposal: <span style="color: ${project.proposalStatus === 'approved' ? '#10b981' : project.proposalStatus === 'rejected' ? '#ef4444' : '#f59e0b'};">${escapeHtml(project.proposalStatus || 'pending')}</span>
                            </div>
                            <div class="work-item-deadline ${project.status}">
                                ${deadlineText}
                                ${daysText ? `<br><small>${daysText}</small>` : ''}
                            </div>
                        </div>
                    </div>
                `;
        ...
        if (user.tasks.length > 0) {
            ...
            sortedTasks.forEach(task => {
                const deadlineText = task.deadline ?
                    ...
                    'No deadline';
                const daysText = task.daysUntilDeadline !== null ?
                    (task.daysUntilDeadline < 0 ?
                        `${Math.abs(task.daysUntilDeadline)} day(s) overdue` :
                        task.daysUntilDeadline === 0 ?
                        'Due today' :
                ...
                const priorityColor = priorityColors[task.priority] || priorityColors['medium'];
                html += `
                    <div class="work-item ${task.status}" onclick="openTaskDetailsModal('${task.id}')">
                        <div class="work-item-header">
                            <div class="work-item-title">
                                ${escapeHtml(task.title)}
                                ${task.hasExtensionRequest ? ' 🔄' : ''}
                            </div>
                            <div class="work-item-type" style="background: ${priorityColor}; color: white;">
                                ${escapeHtml(task.priority.toUpperCase())} PRIORITY
                            </div>
                        </div>
                        ${task.description ? `
                        ...
                        <div class="work-item-meta" style="margin-top: 8px;">
                            <div class="work-item-status">
                                <strong>Status:</strong> ${escapeHtml(task.taskStatus.replace('_', ' '))}
                                ${task.assigneeNames.length > 1 ? ` <span style="color: #94a3b8;">|</span> <strong>Team:</strong> ${task.assigneeNames.length} members` : ''}
                            </div>
                        </div>
                        <div class="work-item-meta" style="margin-top: 8px;">
                            <div style="font-weight: 600; color: #64748b;">
                                Created by: ${escapeHtml(task.creatorName)}
                            </div>
                            <div class="work-item-deadline ${task.status}">
                                ${deadlineText}
                                ${daysText ? `<br><small>${daysText}</small>` : ''}
                            </div>
                        </div>
    ...
    `; // Close team-details section
    html += `
        </div>
    `; // Close report-content-body
    // === RENDER THE REPORT ===
