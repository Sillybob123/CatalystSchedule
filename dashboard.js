...
function applyInitialViewFromUrl() {
    ...
    try {
        ...
        requestedView = (params.get('view') || '').trim().toLowerCase();
    } catch (error) {
        console.warn('[NAV] Could not parse URL parameters:', error);
        return;
    }

    if (!requestedView) return;
    if (requestedView === 'dashboard') requestedView = 'interviews';
    ...
    if (!validViews.has(requestedView)) return;
    handleNavClick(requestedView);
}
...
function renderTasksBoard(tasks) {
    ...
    const columns = [
        ...
        { id: 'completed', title: 'Completed', icon: '🎉', color: '#2563eb' }
    ];
    ...
    orderedTasks.forEach(task => {
        ...
        if (canEditTask) {
            checkbox.addEventListener('change', async (e) => {
                ...
                // --- CHECKLIST GATE ---
                // Before marking key milestones complete, show the editorial checklist.
                if (isChecked && (task === 'Article Writing Complete' || task === 'Review Complete')) {
                    // Revert the checkbox while the modal is open
                    e.target.checked = false;
                    const checklistType = task === 'Review Complete' ? 'editor' : 'writer';
                    const confirmed = await showSubmitChecklist(checklistType);
                    if (!confirmed) {
                        // User cancelled — leave checkbox unchecked, do nothing
                        return;
                    }
                    // User confirmed — restore checked state so the rest of the handler proceeds
...
//  Shown before marking Article Writing Complete
//  or Review Complete
// ================================================
...
/**
 * Shows the submission checklist modal before a key milestone is marked.
 * Returns a Promise that resolves to true (confirmed) or false (cancelled).
 * @param {'writer'|'editor'} type
 * @returns {Promise<boolean>}
 */
function showSubmitChecklist(type) {
    return new Promise((resolve) => {
        const modal = document.getElementById('submit-checklist-modal');
        if (!modal) return resolve(true); // gracefully skip if modal not present

        const titleEl      = document.getElementById('checklist-modal-title');
        const subtitleEl   = document.getElementById('checklist-modal-subtitle');
        const introEl      = document.getElementById('checklist-modal-intro');
        const iconEl       = document.getElementById('checklist-modal-icon');
        const itemsEl      = document.getElementById('checklist-modal-items');
        const warningEl    = document.getElementById('checklist-modal-warning');
        const confirmBtn   = document.getElementById('checklist-modal-confirm-btn');
        const backBtn      = document.getElementById('checklist-modal-back-btn');
        const closeBtn     = document.getElementById('checklist-modal-close-btn');
        const confirmLabel = document.getElementById('checklist-modal-confirm-label');
        const content      = modal.querySelector('.checklist-modal-content');

        const isEditor  = type === 'editor';
        const checklist = isEditor ? EDITOR_CHECKLIST : WRITER_CHECKLIST;

        // Apply color-theme class
        content.classList.toggle('type-editor', isEditor);
        modal.classList.toggle('type-editor',   isEditor);

        // Header text
        titleEl.textContent   = isEditor ? 'Editor Review Checklist' : 'Writer Self-Review Checklist';
        subtitleEl.textContent = isEditor
            ? 'Before marking this article as reviewed, confirm you have completed every item below.'
            : 'Before marking your draft as complete, confirm you have completed every item below.';
        confirmLabel.textContent = isEditor ? 'Confirm Review Complete' : 'Confirm Writing Complete';

        // Header icon (SVG)
        iconEl.innerHTML = isEditor
            ? '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>'
            : '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>';

        // Intro text
        introEl.textContent = isEditor
            ? 'Use this checklist to ensure the article meets Catalyst editorial standards before it moves forward in the workflow.'
            : 'Use this checklist to ensure your draft meets all Catalyst writing standards before your editor reviews it.';

        // Build checklist items (clear first)
        itemsEl.innerHTML = '';
        checklist.forEach((text, i) => {
            const item = document.createElement('div');
            item.className = 'checklist-modal-item';
            ...
            item.innerHTML =
                ...
                '</div>' +
                '<span class="checklist-item-label">' + text + '</span>';
        ...
        function updateConfirmButton() {
            const checkedCount = itemsEl.querySelectorAll('.checklist-modal-item.checked').length;
            const allChecked   = checkedCount === checklist.length;
            confirmBtn.disabled = !allChecked;
            warningEl.style.display = (!allChecked && checkedCount > 0) ? 'flex' : 'none';
        }
        ...
        function onConfirm() {
            const allChecked = itemsEl.querySelectorAll('.checklist-modal-item.checked').length === checklist.length;
            if (!allChecked) { warningEl.style.display = 'flex'; return; }
            close(true);
        }
        ...
        // Clicking outside the modal card = cancel
